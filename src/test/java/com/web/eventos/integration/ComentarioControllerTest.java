...
public class ComentarioControllerTest extends BaseIntegrationTest {
    ...
    @Test
    @DisplayName("Deve salvar comentário")
    void deveSalvarComentario() throws Exception {
    ...
    @Test
    @DisplayName("Deve excluir comentário")
    void deveExcluirComentario() throws Exception {
        ...
        mockMvc.perform(post("/comentarios/" + comentario.getId() + "/excluir")
                .with(user(userDetails))
                .with(csrf()))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/eventos/" + evento.getId()));
